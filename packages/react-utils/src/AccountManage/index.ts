...
type WalletStore = UseBoundStore<
	Write<StoreApi<WalletState>, StoreSubscribeWithSelector<WalletState>>
>;
...
>();
export const getRegisteredWalletsName = () =>
	Array.from(walletsStateMap.keys());
export const getRegisteredWallets = () => Array.from(walletsStateMap.values());
export const registerWallet = (
	walletProvider: WalletProvider,
	{ persistFirst }: { persistFirst: boolean } = { persistFirst: true },
) => {
	...
	walletsStateMap.set(walletProvider.walletName, {
		provider: walletProvider,
		walletStore,
	});
...
export const useBalance = () => {
	...
	const walletState = currentWalletName
		? walletsStateMap.get(currentWalletName)
		: null;
...
export const switchChain = async (
	...
	try {
		await walletState.provider.switchChain?.(
...
function checkWalletState({
	walletName,
	checkFunctionName,
}: { walletName?: string; checkFunctionName?: keyof WalletProvider }) {
	...
	if (
		typeof checkFunctionName === "string" &&
		!!checkFunctionName &&
		typeof walletState?.provider?.[checkFunctionName] !== "function"
	) {
		throw new Error(
			`Wallet - ${currentWalletName} is not support ${checkFunctionName}`,
		);
	}
	return walletState;
}
