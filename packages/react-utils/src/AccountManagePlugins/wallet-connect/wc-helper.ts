import {
	convertBase32ToHex,
	convertHexToBase32,
	hexToCFXAccountHexAddress,
	type Base32Address,
} from "@cfx-kit/dapp-utils/src/address";
...
export const formatChainId = (chainId: string) => {
	if (
		chainId.startsWith(`${ChainPrefix.EIP}`) ||
		chainId.startsWith(`${ChainPrefix.CIP}`)
	) {
		if (chainId.startsWith(ChainPrefix.CIP)) {
			return chainId.replace(
				`${ChainPrefix.CIP}:`,
				`${ChainPrefix.EIP}:${CIPPlaceHolder}`,
			);
		}
		return chainId;
	}
...
export const isCIPData = (data: string) =>
	data.startsWith(`${ChainPrefix.EIP}:${CIPPlaceHolder}`);

export const ExtractCip155Namespace = (
	namespaces: Record<ChainPrefix, Namespace>,
) => {
	...
	const cip155 = {
		chains: namespaces[ChainPrefix.EIP].chains
			?.filter?.(isCIPData)
			.map(convertEipDataToCip),
		accounts: namespaces[ChainPrefix.EIP].accounts
			?.filter?.(isCIPData)
			.map(convertEipDataToCip),
	...
	const eip155 =
		cip155.chains?.length === namespaces[ChainPrefix.EIP].chains?.length
			? null
			: {
					accounts: namespaces[ChainPrefix.EIP].accounts?.filter?.(
						(account) => !isCIPData(account),
					),
					chains: namespaces[ChainPrefix.EIP].chains?.filter?.(
						(chain) => !isCIPData(chain),
					),
					events: namespaces[ChainPrefix.EIP].events,
...
export const mergeCIPNamespaceToEIP = (
	_eipNamespace: Namespace,
	cipNamespace?: Namespace | null,
) => {
	...
	eipNamespace.chains = [
		...new Set(
			(eipNamespace.chains ?? []).concat(
				cipNamespace.chains
					? cipNamespace.chains.map?.(convertCipDataToEip)
					: [],
			),
		),
	];
	eipNamespace.accounts = [
		...new Set(
			(eipNamespace.accounts ?? []).concat(
				cipNamespace.accounts
					? cipNamespace.accounts.map?.(convertCipDataToEip)
					: [],
			),
		),
	];
	eipNamespace.methods = [
		...new Set(
			(eipNamespace.methods ?? []).concat(
				cipNamespace.methods
					? cipNamespace.methods.map?.(convertCipMethodToEip)
					: [],
			),
		),
	];
	eipNamespace.events = [
		...new Set((eipNamespace.events ?? []).concat(cipNamespace.events ?? [])),
	];
	return eipNamespace;
};
