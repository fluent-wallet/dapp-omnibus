...
const ActionTranslate: Translation = {
    ...
    "0xa9059cbb": (arg: TranslationArgs) => {
      ...
      return {
        title: 'Transfer',
        args: parsed.args,
        content:`Transfer ${value} ${symbol} to ${shortenAddress(parsed.args[0])}`
      }
    },
    ...
    "0x23b872dd": (arg: TranslationArgs) => {
      ...
      if (parsed.args[1] === Zero) { // only ERC721
        return {
          title: 'Burn',
          args: parsed.args,
          content: `Burn ${value} ${name} ${symbol}`
        }
      }
      return {
        title: 'Transfer',
        args: parsed.args,
        content: `Transfer ${value} ${symbol} to ${shortenAddress(parsed.args[1])}`
      }
    },
    // approve (ERC20)
    "0x095ea7b3": (arg: TranslationArgs) => {
      ...
      if (parsed.args[1].isZero()) {
        return {
          title: 'Revoked',
          args: parsed.args,
          content: `Revoked ${symbol} to ${shortenAddress(parsed.args[0])}`
        };
      } else {
        // const value = decimals ? formatUnits(parsed.args[1].toString(), decimals) : parsed.args[1].toString();
        return {
          title: 'Approved',
          args: parsed.args,
          content: ` Approved ${symbol} for ${shortenAddress(parsed.args[0])}`
        };
      }
    },
    ...
    "0x42842e0e": (arg: TranslationArgs) => {
      ...
      return {
        title: 'Transfer',
        args: parsed.args,
        content: `Transfer ${value} ${name} ${symbol}`
      };
    },
    ...
    "0xa22cb465": (arg: TranslationArgs) => {
      ...
      if (parsed.args[1] === false) {
        return {
          title: 'Revoked',
          args: parsed.args,
          content: `Revoked ${symbol} from ${shortenAddress(parsed.args[0])}`
        };
      } else {
        return {
          title: 'Approved',
    ...
    "0x2eb2c2d6": (arg: TranslationArgs) => {
      ...
      if (parsed.args[1] === Zero) {
        ...
        return {
          title: 'Mint',
          args: parsed.args,
          content: `Mint ${value} ${name} ${symbol}`
        };
      }
      return {
        title: 'Transfer',
  ...
  const EventTranslate: TranslationEvent = {
    ...
    "0xc3d58168c5ae7397731d063d5bbf3d657854427343f4c083240f7aacaa2d0f62": (arg: EventList) => {
      ...
      return {
        ...
        args: parsed.args,
        content: `TransferSingle ${value} ${name} ${symbol}`
      };
    },
